...
void p3dx_motor_init(P3DX_Motor motors[P3DX_MOTOR_COUNT]) {
  for (int32_t i = 0; i < P3DX_MOTOR_COUNT; ++ i) {
...
void p3dx_motor_set_speed(P3DX_Motor *motor, int32_t duty_cycle) {
  HAL_GPIO_WritePin(motor->dir_gpio_port, motor->dir_pin,
                    (duty_cycle >= 0
                     ? P3DX_MotorDirection_Forward
                     : P3DX_MotorDirection_Backward));
...
inline void p3dx_motor_brake(P3DX_Motor *motor) {
  HAL_GPIO_WritePin(motor->sleep_gpio_port, motor->sleep_pin, GPIO_PIN_SET);
  __HAL_TIM_SET_COMPARE(motor->pwm_timer, motor->pwm_channel, 0);
}

inline void p3dx_motor_coast(P3DX_Motor *motor) {
...
void p3dx_encoder_init(P3DX_Encoder encoders[P3DX_ENCODER_COUNT]) {
  for (int32_t i = 0; i < P3DX_ENCODER_COUNT; ++i) {
    HAL_TIM_Encoder_Start(encoders[i].timer, TIM_CHANNEL_ALL);
    p3dx_encoder_count_reset(&encoders[i]);
...
void p3dx_encoder_update(P3DX_Encoder *encoder) {
  ...
  encoder->ticks = p3dx_encoder_count_get(encoder);
  p3dx_encoder_count_reset(encoder);
}
...
inline void p3dx_encoder_count_reset(P3DX_Encoder *encoder) {
  __HAL_TIM_SET_COUNTER(encoder->timer, (encoder->timer->Init.Period / 2));
}

inline int32_t p3dx_encoder_count_get(const P3DX_Encoder *encoder) {
  return (int32_t)__HAL_TIM_GET_COUNTER(encoder->timer) - (encoder->timer->Init.Period / 2);
}

// ============================================================
// Odometry
void odometry_setpoint_from_cmdvel(P3DX_Odometry *odom, float linear_vel, float angular_vel) {
